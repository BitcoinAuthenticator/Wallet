...
public class BAWalletRestorer extends BASE{
	...
	public BAWalletRestorer(Authenticator auth,WalletRestoreListener l) {      
		...
		listener = l;
		vAuthenticator = auth;
		vWallet = Authenticator.getWalletOperation().getTrackedWallet();
		vBAApplicationParameters = Authenticator.getApplicationParams();
		mainThread = new Thread(){
			@Override
			public void run() {
				directory = new File(auth.getApplicationParams().getApplicationDataFolderAbsolutePath());

				netParams = Authenticator.getWalletOperation().getNetworkParams();
	...
	private void init() throws IOException{
		...
		try {
			 ...
			 */
			File chainFile = new File(directory, Authenticator.getApplicationParams().getAppName() + ".spvchain");
			chainFile.delete();
			vStore = new SPVBlockStore(netParams, new File(directory, Authenticator.getApplicationParams().getAppName() + ".spvchain"));
	...
	private void initWatchedAddresses(int lookaheadForEachAccount) throws Exception{
		Authenticator.getWalletOperation().setHierarchyKeyLookAhead(lookaheadForEachAccount);
		...
		for(ATAccount acc:all){
			...
			for(int i=0; i< lookaheadForEachAccount; i++){
				ATAddress add = Authenticator.getWalletOperation().getNextExternalAddress(acc.getIndex());
				arr.add(add);
				//vorg.authenticator.getWalletOperation().addAddressToWatch(add.getAddressStr()); 
				//addressesToAdd.add(add.getAddressStr());
			}
			mapAccountAddresses.put(acc, arr);
		}
		
		//vorg.authenticator.getWalletOperation().addAddressesToWatch(addressesToAdd);
	}
    				...
    				for(ATAccount acc:keys){
    					...
    					for(ATAddress add:addresses){
    						if(addr.toString().equals(add.getAddressStr())){
    							Authenticator.getWalletOperation().markAddressAsUsed(acc.getIndex(), add.getKeyIndex(), HierarchyAddressTypes.External);
    							ATAddress newAdd = Authenticator.getWalletOperation().getNextExternalAddress(acc.getIndex());
    							mapAccountAddresses.get(acc).add(newAdd);
    							Authenticator.getWalletOperation().addAddressToWatch(newAdd.getAddressStr()); 
