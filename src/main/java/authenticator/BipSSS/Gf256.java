...
public class Gf256 {
    ...
    private byte[][] sha256Coefficients(byte[] secret, int m) throws IOException {
        byte[][] res = new byte[m][];
        byte[] coeff = copyOfRange(secret,0,secret.length);
        res[0] = coeff;
        for (int n = 1; n < m; n++) {
        	ByteArrayOutputStream writer = new ByteArrayOutputStream(coeff.length + 32);
            //ByteWriter writer = new ByteWriter(coeff.length + 32);
            for (int i = 0; i < coeff.length; i += 32) {
                int toHash = Math.min(32, coeff.length - i);
                writer.write(sha256(coeff, i, toHash).getBytes());
            }
            coeff = copyOfRange(writer.toByteArray(), 0, coeff.length);
            res[n] = coeff;
        }
        return res;
    }
     ...
     * 
     */
    public Sha256Hash sha256(byte[] data, int offset, int length) {
     ...
     * @throws IOException 
     */
    public List<Share> makeShares(byte[] secret, int threshold, int shares) throws IOException {
